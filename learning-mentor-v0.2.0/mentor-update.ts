...
const API_LATEST =
	process.env.LEARNING_MENTOR_API ||
...
export function unifiedDiff(
	...
	for (const group of groups) {
		...
		out.push(
			`@@ -${formatRange(first[1], tail[2])} +${formatRange(first[3], tail[4])} @@`,
		)
...
async function fetchLatest(
	force = false,
): Promise<[Latest | null, string | null]> {
	const cachePath = pathInHome("cache.json")
	const cache = readJson(cachePath, {}) ?? {}

	if (!force) {
		const age = hoursSince(cache.checked_at)
		if (age !== null && age < CHECK_INTERVAL_HOURS && cache.latest)
			return [cache.latest, null]
		const failAge = hoursSince(cache.failed_at)
		if (failAge !== null && failAge < FAILURE_INTERVAL_HOURS && cache.error)
			return [null, cache.error]
	}
...
export function decideNotice(state: NoticeState): string | null {
	...
	const usedRecently =
		state.hook_last_run_hours !== null && state.hook_last_run_hours < 72
...
async function cmdHook(): Promise<number> {
	...
	try {
		...
		stamp.ok = false
		stamp.error =
			exc instanceof Error ? `${exc.name}: ${exc.message}` : String(exc)
	}
...
async function cmdCheck(): Promise<number> {
	...
	if (!receipt) {
		...
		console.log(
			`  ${commandLine(scriptPath(), "--setup --tool claude-code --target agent:/絶対パス/learn.md")}`,
		)
		return 1
	}
	...
	console.log(
		`配置済み : v${local}（${receipt.installed_at ?? "日付不明"} に配置）`,
	)
	console.log(
		`運用     : ${receipt.tool ?? "不明"} / 運用型 ${receipt.pattern ?? "不明"}`,
	)
	console.log("配置先   :")
	for (const target of receipt.targets ?? []) {
		...
		else if (stamped !== local)
			note = `★ 受領書は v${local} だがファイルは v${stamped}`
		else note = "OK"
		// 複数リポジトリに配置すると、配置先ごとに別のツールを使っていることがある
		const kind = target?.kind ?? "?"
		const label =
			target?.tool && target.tool !== "unknown"
				? `${target.tool}/${kind}`
				: kind
	...
	if (!stamp) {
		console.log(
			"hook     : ★ 一度も動いていません。hook の設定が効いていない可能性があります",
		)
	} else {
		const age = hoursSince(stamp.ran_at)
		const when = age !== null ? `${age.toFixed(1)} 時間前` : "不明"
		if (!(stamp.ok ?? true))
			console.log(`hook     : ★ 最後の実行で失敗 — ${stamp.error}（${when}）`)
		else if (age !== null && age > 24 * 7)
			console.log(
				`hook     : ★ ${when} から動いていません。設定が外れた可能性があります`,
			)
	...
	if (isNewer(latest, local)) {
		...
		console.log("")
		console.log(
			`更新するには: ${commandLine(receipt.script_path ?? scriptPath(), "--apply")}`,
		)
		return 2
	}
...
async function downloadZip(
	...
	try {
		...
		if (!response.ok)
			return [null, `zip を取得できませんでした (HTTP ${response.status})`]
...
async function cmdApply(args: Args): Promise<number> {
	...
	if (!latestInfo.asset) {
		console.log(
			`リリース v${latest} に zip アセットが付いていません: ${latestInfo.url}`,
		)
		return 1
	}
	...
	for (const [path, original, updated] of plans) {
		...
		console.log(`  ${path}`)
		console.log(
			`      +${added} 行 / -${removed} 行（マーカーの外は変えません）`,
		)
	...
	if (
		!args.yes &&
		!confirm(`上記を v${latest} に更新します。よろしいですか [y/N]: `)
	) {
		console.log("中止しました。")
		return 1
	}
	...
	for (const [path, original, updated] of selfPlans) {
		try {
			...
			console.log(
				`   zip を落とし直して手で置き換えてください: ${latestInfo.url}`,
			)
		}
	}
...
const HOOK_TARGETS: Record<
	string,
	{ settings: string[]; matcher: string; fields: Json }
> = {
	"claude-code": {
 ...
 */
function consolidateSessionStart(
	groups: Json[],
	desired: Json,
): [Json[], number] {
 ...
 */
function installHook(tool: string | undefined): {
	status: string
	lines: string[]
} {
	...
	if (raw === text) {
		return {
			status: "already",
			lines: [`hook     : 設置済み（${path}）`, ...ensureCodexFeature(tool)],
		}
	}
	...
	try {
		...
		return {
			status: "failed",
			lines: [`hook     : ★ ${path} に書けませんでした — ${errorText(exc)}`],
		}
	}
	...
	const others = updated
		...
		.reduce(
			(n: number, g: Json) =>
				n + (Array.isArray(g?.hooks) ? g.hooks.length : 0),
			0,
		)
	const lines = removed
		? [
	...
	if (others)
		lines.push(
			`           ほかの SessionStart 設定 ${others} 件はそのまま残しています`,
		)
...
function ensureCodexFeature(tool: string): string[] {
	...
	if (/^\s*codex_hooks\s*=\s*true/m.test(text))
	...
	return [
		`           ${path} に [features] codex_hooks = true を足しました（元は .bak）`,
	]
}
...
function alreadyIgnored(existingLines: string[], entry: string): boolean {
	const stripped = new Set(
		existingLines.map((line) => line.trim().replace(/^\/+/, "")),
	)
...
function updateGitignore(
	...
	const existingEntries = blockBody.filter(
	...
	for (const entry of entries) {
		if (
			...
			!alreadyIgnored([...before, ...merged, ...after], entry)
		) {
			merged.push(entry)
		}
	}
	...
	if (
		beginAt !== -1 &&
		merged.length === existingEntries.length &&
...
function protectTargetsFromGit(targets: Json[]): string[] {
	...
	for (const [root, paths] of byRepo) {
		const [gpath, changed] = updateGitignore(
			root,
			paths.map((p) => gitignoreEntry(root, p)),
		)
		lines.push(
			changed
				? `gitignore: ${gpath} に追記しました`
				: `gitignore: ${gpath} は既に対応済みです`,
		)
	}
	...
	for (const path of warnEmbedded) {
		lines.push(
			`gitignore: ★ ${path} は既存ファイルへの同居配置（embedded）のため、自動では除外できません`,
		)
		lines.push(
...
function printVerification(): void {
	...
	console.log("")
	console.log(
		"  置き場所を間違えても、エラーは出ません。黙って無視されるだけです。",
	)
	...
	console.log(
		"  Edit / Write やシェル実行が入っている       → 失敗。置き場所か起動方法が違います",
	)
	console.log("")
	console.log(
		"  「書き換えて」と頼んで断られたかどうかでは判定できません。ツールが無くても",
	)
	console.log(
		"  AIは「〜という設定なので、できません」と説明するだけで、区別がつかないためです。",
	)
	console.log("")
	console.log(
		"  hook が本当に動いたかは、セッションを1回起動したあとに分かります:",
	)
...
function cmdSetup(args: Args): number {
	...
	if (added && merged.length > added) {
		console.log(
			`  配置先 ${merged.length} 件（今回 ${added} 件を追加。既存の登録は残しています）`,
		)
	...
	for (const target of merged) {
		...
		if (!existsSync(target.path)) {
			console.log(
				`  ★ ${target.path} が見つかりません（--apply の対象から外れています）`,
			)
		} else if (stamped === null) {
			console.log(
				`  ★ ${target.path} にマーカーがありません。本文を次の形で囲んでください:`,
			)
